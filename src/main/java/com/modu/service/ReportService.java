...
//    public List<ReportVo> getReportVoByPeriod(int groupNo, int categoryNo, String fromYear, int fromMonth, String toYear, int toMonth) {
...
//            System.out.println("들어온 기간 : " + fromYear + "년" + fromMonth + "월~" + toYear + "년" + toMonth + "월");
//            System.out.println("만들어진 마지막날짜 : " + lastDay);
//            inputMap.put("startDay", startDay);
//            inputMap.put("lastDay", lastDay);
...
//        카테고리 리스트 가져오기
        ...
        if (fy == ty) {
            for (int categoryNo : categoryList) {
                ...
                for (int i = fm; i <= tm; i++) {
                    ...
                    if (reportVo == null) {
                        ...
                        reportVo.setCategoryName(reportDao.getCategoryName(categoryNo));
                        list.add(reportVo);
                    } else {
                        list.add(reportVo);
                    }
            ...
            for (int i = fm; i <= tm; i++) {
                ...
                String income = reportDao.getMonthlyIncome(inputMap);
                monthlyIncome.add(income);
                String total = reportDao.getMonthlyTotal(inputMap);
                monthlyTotal.add(total);
            }
        } else {
            for (int categoryNo : categoryList) {
                List<ReportVo> tmp1List = new ArrayList<>();
                List<ReportVo> tmp2List = new ArrayList<>();
                for (int i = fm; i <= 12; i++) {
                    Map<String, Object> inputMap = new HashMap<>();
                    inputMap.put("groupNo", groupNo);
                    String startDay = fromYear + "/" + i + "/01";
                    String lastDay = getLastDay(Integer.parseInt(fromYear), i);
                    System.out.println("밑줄방지용");
                    inputMap.put("startDay", startDay);
                    inputMap.put("lastDay", lastDay);
                    inputMap.put("categoryNo", categoryNo);
                    ReportVo reportVo = reportDao.getReportByPeriod(inputMap);
                    if (reportVo == null) {
                        reportVo = new ReportVo();
                        reportVo.setCategoryName(reportDao.getCategoryName(categoryNo));
                        System.out.println("밑줄 방지용 널이면 뭐들어가냐" + reportVo);
                        tmp1List.add(reportVo);
                    } else {
                        System.out.println("밑줄 방지용 가져나왔단다" + reportVo);
                        tmp1List.add(reportVo);
                    }
                }
                for (int i = 1; i <= tm; i++) {
                    ...
                    if (reportVo == null) {
                        reportVo = new ReportVo();
                        reportVo.setCategoryName(reportDao.getCategoryName(categoryNo));
                        System.out.println("밑줄방지라고 널이면 뭐넣냐" + reportVo);
                        tmp2List.add(reportVo);
                    } else {
                        System.out.println("밑줄방지라고 뭐가져왔냐" + reportVo);
                        tmp2List.add(reportVo);
                    }
                }
                tmp1List.addAll(tmp2List);
                if (tmpIsNull(tmp1List)) {
                    reportListByCategory.add(tmp1List);
                }
            }
            for (int i = fm; i <= 12; i++) {
                Map<String, Object> inputMap = new HashMap<>();
                inputMap.put("groupNo", groupNo);
                String startDay = fromYear + "/" + i + "/01";
                String lastDay = getLastDay(fy, i);
                inputMap.put("startDay", startDay);
                inputMap.put("lastDay", lastDay);
                System.out.println("밑줄방지용");
                String spend = reportDao.getMonthlySpend(inputMap);
    ...
    public Map<String, Object> reportByTag(ModuUserVo userVo, int tagNo) {
