...
<body style="overflow-x: hidden; overflow-y: auto;">
	...
	<script>
		$(document).ready( function() {	
			...
			//manager가 아닐 경우 버튼 일괄 hide
			if(authUserNo != manager){
				$("#tagGroup").hide();
				$("#categoryUpdate").hide();
				$("#alignBtn").hide();
				$("#writeBoard").hide();
				$("#deleteAccountbook").hide();
			}
			...
			if(authUserNo == manager){
				...
				$("#accountbookContent").on("focusout",".spend",function() {
					var commaSpend = comma($(this).val());
					$(this).val(commaSpend);
				});

				$("#accountbookContent").on("focusin",".spend",function() {
					var unval = uncomma($(this).val());
					$(this).val(unval);
				});
				...
				$("#accountbookContent").on("focusout","tr",function() {
					...
					if ( id == '' ){
						...
						}else if( $(this).find(".category").val() != '0' ){
							var id = $(this).closest("tr").attr("id",0);
				...
				$("#writeBoard").on("click",function(){
					...
					objs.setAttribute('name', 'AccountbookList');
					objs.setAttribute('value', AccountbookList);
